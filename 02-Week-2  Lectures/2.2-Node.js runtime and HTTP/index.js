// 2.2 Node.js runtime and HTTP  Date:22/02/24 Time:10:07 PM
...
// ->A place where request goes is backend server.
...
// ->remote procedure calls - Remote Procedure Call is a software communication protocol that one program can use to request a service from a program located in another computer on a network without having to understand the network's details.
...
// ->Whenever server recives a request it serves the information to the client with some Response headers, Reponse body, and Status codes.
...
// 1.Response header
// ->server sends request header while sending information to client.
...
// 4.7 What are the common methods send to Backend server?
...
// ->we are going write in node.js and use library called express.
...
// eg-1: creating HTTP server using node.js and Express.
...
// 3. When multiple request are sent to node.js util one request completed it will not go other.
...
// ->vs code - server
// ->postman - client
// ->accessing we can access data from server in our devices if they are connected to same network using IP.
...
// ->by express parse body. we can also tell in which form it has to parse(text)

// 7. user is hitting function on sepcidic url and route.
// eg:
// app.post("/route",function(req,res){
...
// ->it take over a port.
// ->when port is given to listen() method in express it writes a logic to create a http server that listens on the port.
